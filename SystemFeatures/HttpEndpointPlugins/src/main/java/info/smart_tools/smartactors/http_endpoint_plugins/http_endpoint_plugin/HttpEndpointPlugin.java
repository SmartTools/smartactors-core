...
public class HttpEndpointPlugin implements IPlugin {
                        ...
                        try {
                            ...
                            IOC.register(
                                    ...
                                    new ApplyFunctionToArgumentsStrategy(
                                            (args) -> {
                                                try {
                                                    return IOC.resolve(Keys.getKeyByName(IObject.class.getCanonicalName()));
    ...
    private void registerExceptionalResponse() throws InvalidArgumentException, ResolutionException, RegistrationException {
        IObject er400 = IOC.resolve(
                Keys.getKeyByName(IObject.class.getCanonicalName()),
                "{\"exception\": \"Request body is not json\", \"statusCode\": 400}"
        );
        IObject er404 = IOC.resolve(
                Keys.getKeyByName(IObject.class.getCanonicalName()),
                "{\"exception\": \"This url is not registered\", \"statusCode\": 404}"
        );
        IObject er500 = IOC.resolve(
                Keys.getKeyByName(IObject.class.getCanonicalName()),
                "{\"exception\": \"Internal server error\", \"statusCode\": 500}"
                );
        IObject er503 = IOC.resolve(
                Keys.getKeyByName(IObject.class.getCanonicalName()),
                "{\"exception\": \"Service not available (shutting down)\", \"statusCode\": 503}"
                );

        IOC.register(
                Keys.getKeyByName("HttpPostParametersToIObjectException"),
                new SingletonStrategy(er400)
        );
        IOC.register(
                Keys.getKeyByName("HttpRequestParametersToIObjectException"),
                new SingletonStrategy(er404)
        );
        IOC.register(
                Keys.getKeyByName("HttpInternalException"),
                new SingletonStrategy(er500)
        );
        IOC.register(
